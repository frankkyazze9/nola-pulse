...
import { listProjects } from "@/lib/brain/handlers";
...
export default async function ProjectsPage({
  ...
  const projects = await listProjects({ status: filter, limit: 100 });
  ...
  return (
    <div className="max-w-6xl mx-auto w-full px-4 py-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted mt-1">
            Campaigns and brands — {projects.length} {filter ?? "total"}
          </p>
        </div>
        <Link
          href="/research"
      ...
      <div className="flex gap-2 mb-4 overflow-x-auto">
        <FilterLink href="/projects" active={!filter}>
          All
        </FilterLink>
        {STATUSES.map((s) => (
          <FilterLink
      ...
      {projects.length === 0 ? (
        <div className="px-4 py-12 border border-dashed border-card-border rounded text-center">
          ...
          <p className="text-xs text-muted">
            Ask the agent on the Research page to start a campaign or brand
            project.
          </p>
        </div>
      ) : (
